 ...
 *
 */
public class ActivityCustomerSelect extends Activity{
	
	private static final int HANDLER_MSG_SYNC_CUSTOMER 			= 1; 
	private static final int HANDLER_MSG_CUSTOMER_SELECT 		= 1 + HANDLER_MSG_SYNC_CUSTOMER; 
	private static final int HANDLER_MSG_UPDATE_LISTVIEW 		= 1 + HANDLER_MSG_CUSTOMER_SELECT; 
	...
	private Handler handler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			switch(msg.what){
			case HANDLER_MSG_SYNC_CUSTOMER:
				queryCustomer();
				break;
				
			case HANDLER_MSG_CUSTOMER_SELECT:
				
				adapter = new ListAdapterRadioKVPair(context, listArray,WebApi.API_PARAM_CUSTOMER,
						WebApi.API_PARAM_CUSTOMER_SERIAL,lsrRadioButton);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		handler.sendEmptyMessage(HANDLER_MSG_SYNC_CUSTOMER);
	}
	...
	private OnClickListener lsrButton = new OnClickListener(){
		...
		public void onClick(View v) {
			...
			case R.id.buttonSet:
				...
				if(position>=0){
					StaticData.customer_serial = (int) adapter.getItemId(position);
				}
				else{
					StaticData.customer_serial = 0;
				
				}
				
				if(StaticData.customer_serial>0){
					StaticData.customer = adapter.getItemName(position);
				}
				
				intent = new Intent();
				if(StaticData.customer_serial<=1){
	...
	private OnHttpResponse onHttpRespCustomerQuery = new OnHttpResponse(){
		...
		public void onHttpResponse(String response) {
			...
			if(WebApi.isRespSuccess(response)){
				try{
					...
					obj.put("customer_serial", 0);
					obj.put("customer", "全部客户");
					listArray.add(obj);
					handler.sendEmptyMessage(HANDLER_MSG_CUSTOMER_SELECT);
	...
	private void queryCustomer(){
		...
		webapi.setOnHttpResponse(this.onHttpRespCustomerQuery);
		webapi.queryCustomer();
	}	
