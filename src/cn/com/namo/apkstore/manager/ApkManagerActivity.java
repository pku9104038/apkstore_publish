...
/*				
				...
				intent.putExtra(ActivityRadioSelect.INTENT_EXTRA_OPTIONS, PrefProxy.getCategoryList(context));
				intent.putExtra(ActivityRadioSelect.INTENT_EXTRA_VALUE_NAME, "category_serial");
				intent.putExtra(ActivityRadioSelect.INTENT_EXTRA_TEXT_NAME, "category");
				intent.putExtra(ActivityRadioSelect.INTENT_EXTRA_TITLE, apkinfo.getLabel());
				
				((ApkManagerActivity) context).startActivityForResult(intent, ApkInfo.ONLINE_STATE_UNKNOWN);				
				*/
				break;
...
/*				
				...
				intent.setClass(context, ActivityRadioSelect.class);
				intent.putExtra(ActivityRadioSelect.INTENT_EXTRA_INDEX, index);
				intent.putExtra(ActivityRadioSelect.INTENT_EXTRA_OPTIONS, PrefProxy.getSupplierList(context));
				intent.putExtra(ActivityRadioSelect.INTENT_EXTRA_VALUE_NAME, "supplier_serial");
				intent.putExtra(ActivityRadioSelect.INTENT_EXTRA_TEXT_NAME, "supplier");
				intent.putExtra(ActivityRadioSelect.INTENT_EXTRA_TITLE, apkinfo.getLabel());
				
				((ApkManagerActivity) context).startActivityForResult(intent, ApkInfo.ONLINE_STATE_APP_ONLINE);				
			...
			if(resultCode == Activity.RESULT_OK && data != null){
				...
				if (index > -1 && supplier_serial > 0 ){
					...
					webApi.addApkinfo(
							...
							0,//StaticData.customer_serial,
							0,//StaticData.brand_serial,
							0//StaticData.model_serial
							);
...
/*					
	...
	private void syncApkInfos(){
		...
		webapi.syncApkInfos(listApkInfo);
	}
	...
	private Handler handler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			case HANDLER_MSG_APP_ADD:
				...
				((ApkManagerActivity) context).startActivityForResult(
						makeOptionSelectIntent(index,
								...
								"选择应用类型:\n"+listApkInfo.get(index).getLabel(),
								PrefProxy.getCategoryList(context),
								ActivityRadioSelect.VALUE_NAME_CATEGORY_SERIAL,
								ActivityRadioSelect.TEXT_NAME_CATEGORY
								), 
			...
			case HANDLER_MSG_APK_UPLOAD:
				...
				/*
				((ApkManagerActivity) context).startActivityForResult(
						makeUploatFiletIntent(index,listApkInfo.get(index).getFilePath()), 
						ApkInfo.ONLINE_STATE_APK_ONLINE);
				*/						
